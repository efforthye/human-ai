...
### [8] 테스트 데이터에서 재구성 오차 계산
```py
# 테스트 데이터를 autoencoder에 통과시켜서 복원값을 얻어옴
X_test_pred = autoencoder.predict(X_test_scaled)
# 각 샘플에 대해서(입력 - 복원값)^2를 계산 (특징 차원방향으로 평균을 내서 재구성 오차를 구하는 공식)
