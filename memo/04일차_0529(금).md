...
```
one_answer_df.to_csv('./results/3-05)one_answer.csv', index=False)
two_answer_df.to_csv('./results/3-05)two_answer.csv', encoding='utf-8-sig') # utf-8-sig: 한글 깨짐 방지
np.savetxt('./results/3-05)three_answer.csv', [three_answer], fmt='%.5f', delimiter=',')
```

## 문제4-교수님풀이) 결과를 csv 파일로 저장하기.
```
bad_loan_status_to_grades.sort_index().to_csv('./results/3-05)bad_loan_status_교수님.csv', sep=',')
```
...
## csv 저장
```py
df.to_csv('./results/4-01)sales_data.csv', index=False, encoding='utf-8-sig')
print("4-01)sales_data.csv 파일이 생성됐습니다.")
print(df.head())
```
## csv 파일 읽어오기 - read_csv
```py
df = pd.read_csv('./results/4-01)sales_data.csv')
...
- 1.5) Class 별 과목 분석
    ...
    - pairplot 그래프 상관관계 해석: 대각선에 있는 산모양 그래프는 해당 과목의 점수분포이다.
    - 대각선 외 나머지 칸은 두 과목씩 비교해서 과목 간 산점도를 나타낸 것이다. 각 산점도는 다음과 같이 해석 가능하다.
        - kor-eng(우상향): 국어 잘하면 영어도 잘하는 경향
        - eng-math(우상향): 영어 잘하면 수학도 잘하는 경향
        - math-science(우상향): 수학 잘하면 과학도 잘하는 경향
        - kor-science(우하향): 국어 잘하면 과학은 오히려 낮은 경향
        - 데이터가 적어서(10개) 신뢰는 낮다.

# 그 외 AI 프로세스 설명
![alt text](image-125.png)
- 원천데이터 실시간 처리 -> 원천 디비 저장 -> 전처리/가공 -> 학습데이터 -> 학습데이터 디비에 저장 -> 그 데이터를 읽어서 학습 -> 학습 결과를 추론 모듈에 배포 -> 실시간으로 웹에서 신규데이터 분석요청 들어오면 추론모듈이 학습결과 기반으로 인퍼런스 추론을 해서 결과를 전달한다.
- 실시간인 부분, 배치프로세스인부분, 실시간인 부분 이렇게 흘러간다. 나중엔 이런 시스템 구축을 할 것이다.
- 내일 실습: 데이터 발생하면, 디비(몽고디비/nosql)에 저장하고 -> 임의의 모델을 만들어 실행시킬 것이다. -> 그 데이터를 읽어 학습하고 신규데이터 들어오면 추론 & 다시 디비에 저장하는 실습을 할 것이다. (몽고디비)
- 원래 원천데이터와 원천디비 사이 통신 들어감, 웹에서도 분석요청하고 결과 받는곳에 통신, 학습하고 추론 하는 사이에 통신 들어간다. 내일 통신 실습도 진행할 것이다(TCP/IP&FAST API)
- 최근에는 하드웨어와 그 사이 통신하는 MQTT 프로토콜이 표준으로 되어있다. 내일 하거나 프로젝트때 실습할 것이다. (참고)
- 내일부터는 실무적인 실습 들어갈 것임.
