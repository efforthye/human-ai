...
sample_index = 0
# 개별 샘플 shap 값
shap.initjs() # SHAP 자바스크립트 시각화 엔진 초기화 (force_plot 등 인터랙티브 그래프용)

# 특정 샘플 하나(sample_index)에 대해 각 feature가 예측에 기여한 정도를 폭포수(waterfall) 그래프로 표시
shap.waterfall_plot(
    shap.Explanation(
        values=shap_values[sample_index], # 해당 샘플의 feature별 SHAP 값
        base_values=explainer.expected_value, # 기준값(전체 평균 예측, 출발점)
        data=X_test.iloc[sample_index], # 해당 샘플의 실제 feature 값
        feature_names=X.columns # feature 이름
    )
)

# Force plot (interactive)
# 첫 번째 샘플(0번)에 대해 각 feature가 예측을 어느 방향으로 밀었는지 force plot으로 표시
shap.force_plot(
    explainer.expected_value, # 기준값(평균 예측)
    shap_values[0], # 0번 샘플의 SHAP 값
    X_test.iloc[0] # 0번 샘플의 실제 feature 값
)

# shap 값 직접 확인
# 전체 샘플의 SHAP 값을 feature를 컬럼으로 하는 표(DataFrame)로 변환
shap_df = pd.DataFrame(shap_values, columns=X.columns)

print("\n[SHAP 값 샘플]")
print(shap_df.head())  # 상위 5개 샘플의 SHAP 값 확인
```
![alt text](image-379.png)
![alt text](image-414.png)
![alt text](image-413.png)
- SHAP으로 위와 같은 결과를 확인할 수 있다.
...
# 세 가지 기준의 순위를 합산하여 종합 점수 계산 (토탈 점수를 매긴 컬럼 추가)
...
- 정상에서 상위 15% 수준을 워닝 라인으로 잡도록 바꾸니까 warning 라인과 critical 라인이 정상적으로 보이게 됨: ![alt text](image-402.png)

#### [중요] 나중에 이렇게 그래프 정상 데이터 이상 데이터 영향 미쳤던 변수들 타임싱크 맞춰서 같이 상태 어떻게 나타나는지 비교해서 보라고 한다.

#### 트레이서빌리티(traceability) 분석에 관하여
- 원래는 트랜서빌리티 분석(근원 인자가 무엇인지 찾아가는 분석) 도 하는데 이선 실습에서는 진행 안함 (근원 추적은 보통 더 복잡한 작업이 필요)
    - 각 이상 샘플에서 어떤 치명인자가 크게 기여했는지 분해 (개별 예측에 대한 기여도 분석)
    - 앞서 만든 고장 유형 분류 모델로 원인 유형을 매핑
    - 변수 간 인과 관계 그래프를 그리는 식의 추가 단계가 들어감
- 아무튼, 실제 현업에서는 이상이 났으면 -> 왜 났는지 근원까지 추적하는 단계까지 진행하기도 한다. (현재는 이상을 잡아내고 위험도를 매기는 정도까지 했음)

### [12] 원인 분류 모델 학습 (이상 데이터 기반)
-  이상으로 판정된 데이터만 모아서 어떤 종류의 고장인지 맞춤
```py
# 이상 데이터 프레임 정의
abnormal_df = df_feat[df_feat.is_abnormal==1]
# 이상 데이터 중 치명 인자만 입력 변수로 선택
X_cause = abnormal_df[critical_cols]
# 이상 데이터의 고장 유형을 fault_type에 있는 항목으로 정답 라벨로서 사용
y_cause = abnormal_df.fault_type

# 학습용/평가용 데이터 분리 (75%:25%), stratify로 고장 유형 비율을 양쪽에 동일하게 유지
X_train, X_test, y_train, y_test = train_test_split(X_cause, y_cause, test_size=0.25, random_state=42, stratify=y_cause)
# 원인 분류 모델 정의 (랜덤 포레스트 사용)
cause_model = RandomForestClassifier(n_estimators=400, max_depth=None, random_state=42)
# 원인 분류 모델 학습
cause_model.fit(X_train, y_train)

# 학습하지 않은 X_test 데이터를 가지고 고장 원인 예측
y_pred = cause_model.predict(X_test)
# 학습한 데이터 성능 검증 (고장 원인 분류 성능 출력)
print(classification_report(y_test, y_pred))

# confusion matrix 계산
cm = confusion_matrix(y_test, y_pred, labels=cause_model.classes_)

# 그래프 출력
plt.figure(figsize=(7,5))
sns.heatmap(
    cm, # 계산된 confusion matrix
    annot=True, 
    fmt="d",
...
- 예측을 했다.
- 3가지 클래스 중 0.98, 1 잘 나왔다. (나는 0.97, 1 이렇게 나옴)
- 신뢰도 점수를 계산했다.
- 239개 맞췄고 11개 틀렸고 (245개 맞추고, 5개 틀렸으면 양호한 편이다.)
- 이제 실제 원인 분류 3가지 고장 클래스 -> 어떤 인자가 가장 영향을 많이 미쳤었는지 뽑아볼 것이다.

### [13] 원인 분류 모델에서 변수 중요도 확인
- 피쳐 임포턴스 기반으로 뽑을 것이다.
```py
# 원인분류 모델의 결과를 기반으로 변수 중요도 계산.
cause_importance = pd.Series(cause_model.feature_importances_, index=critical_cols).sort_values(ascending=False)
...
- 랜덤포레스트가 x/y 기반으로 학습 -> ... -> 파생변수로 비교
- 랜덤포레스트에서는 y에 영향 미쳤던 변수가 이렇게 우선순위로 나온다.
- 상태값 기반으로 변수의 중요도 파악.
- x_test 고장유형이 나왔으면 -> 그 고장 유형에 가장 영향을 미쳤었던 인자들이 이 그래프의 인자들인 것이다.
- 학습하지 않았었던 신규 데이터(카프카 실습에서 실시간 들어오는 인풋 데이터) -> 어떤 원인인지/어떤 문제인지 이제 진단하는 것을 만들 것이다.

### [14] 신규 데이터 생성 및 테스트 (SHAP)
- 설비에서 신규 데이터 로우데이터가 들어옴. 이것도 파생변수 했던 것처럼 동일하게 맞춰 주어야 함.
```py
# 신규 샘플 데이터 생성 (시나리오)
# 1번: 베어링 마모 의심되는 샘플
# 2번: 유압 누설 의심되는 샘플
...
# 기존 파생변수 생성 함수를 활용하여 신규 데이터에 동일한 파생변수 생성 
# - 기존에 파생변수 생성했을 때처럼 타입을 맞춰 주어야 하기 때문
new_feat = add_critical_features(new_data)
# 파생변수 정상 기준 스케일러로 변환
new_scaled = scaler.transform(new_feat[critical_cols])
# 신규 데이터 이상점수 계산
new_score = -iso_model.decision_function(new_scaled)

# 신규 데이터에 이상 점수 추가
new_feat["anomaly_score"] = new_score
# 신규 데이터에 리스트 레벨 추가
new_feat["risk_level"] = new_feat.anomaly_score.apply(lambda x: classify_risk_level(x, warning_threshold, critical_threshold))
# 신규 데이터에 위험 등급 추가
new_feat["predicted_cause"] = "normal" # 기본값: normal로 설정

# Warning or Critical 데이터만 원인 분류 대상으로 선택 (NORMAL이 아닌 것)
abnormal_mask = new_feat.risk_level != "NORMAL"
# [중요] 이상으로 판단된 신규 데이터에 대해서만 고장 원인 예측 수행하도록 설정
new_feat.loc[abnormal_mask, "predicted_cause"] = cause_model.predict(new_feat.loc[abnormal_mask, critical_cols])
# 최종 진단 결과 출력
print(new_feat[critical_cols+["anomaly_score", "risk_level", "predicted_cause"]])
```
![alt text](image-406.png)
- 샘플1, 샘플2, 샘플3 에 대한 각각의 어노멀리 점수가 나오고 셋다 크리티컬로 나왔다.
- 이유(predicted_cause) 도 각각 진단되어 나왔다.
- 시각화를 해도 되지만 일단 이것만 봐도 잘 보이기 때문에 넘어감.

### [15] 신규 데이터 원인 별 치명 인자 프로파일 시각화
```py
# 신규 데이터의 치명 인자만 복사해둔다.
new_profile = new_feat[critical_cols].copy()
# 
...
- x축 = 샘플데이터 3개 만든 것.
- 고장 유형은 1. 베어링마모, 2. hydraulic_leak, 3. lubrication_fault 이었다.
- 주요 인자에 영향 미친 것들이 빨갛게 영향 미쳤다.
- 샘플1은 1.2,1.2,1.3 인 항목이 영향 미쳤다: ![alt text](image-411.png)
- 샘플2는 마지막 빨간게 (1.3) 영향 미친 것이고: ![alt text](image-410.png)
- 샘플3은 아래 3개가 영향 미친 것이다: ![alt text](image-409.png)

#### 오늘 실습에는 각 단계별 활용할 내용들이 많이 들어갔다.
- 이런 내용은 어떤 도메인이든 다 적용이 된다.

## 2. 유방암 진단 (XGBoost & SHAP)
- https://github.com/ilovejwoo1004-bit/AI_advanced/blob/main/%EB%A8%B8%EC%8B%A0%EB%9F%AC%EB%8B%9D/XGBoost%20%26%20SHAP2.ipynb
- 위 코드 다운받아서 실행해보기
- 값이 클수록 양성(정상), 작으면 음성
- 현업 담당자는 이 원인이 뭐때매 일어나는지 안다고 한다.
- SHAP 이 더 변수 기여도를 정확하게 찾아냈다고 했다.
- 우리 모델 결과 해석할 때도 SHAP 벨류 기반으로 많이 진행할 것이다.
...
#### XGBoost 핵심 특징
1. Boosting 방식: 하나의 트리를 학습한 후 다음 트리는 이전 트리의 오차를 보완하는 방향으로 학습. 즉, "틀린 부분을 계속 보완하며 강한 모델" 생성
2. Gradient Boosting 기반: 각 반복(iteration)에서 Loss Function(오차)을 줄이도록 Gradient 값을 사용해 트리가 생성됨.
3. 규제(Regularization) 포함: 기존 GradientBoosting과 달리 L1, L2 규제를 포함 -> 과적합(Overfitting)을 방지하는 효과
4. 분산 처리 + 빠른 연산: 자체적인 최적화(병렬 처리, sparsity 대응 등)로 학습 속도가 매우 빠름
5. Early Stopping 지원: 검증셋의 성능이 일정 rounds 동안 좋아지지 않으면 학습 자동 종료 -> 시간 절약 + 과적합 방지
#### XGBoost의 동작 과정 요약
- 첫 번째 트리가 데이터 패턴을 학습
- 예측 오류(residual error = 실제 - 예측)를 계산
- 두 번째 트리는 이 오류를 줄이도록 학습
- 계속 반복하며 트리가 추가됨
- 모든 트리의 예측값을 합쳐 최종 예측 -> 약한 트리들이 모여 강한 예측기가 됨

#### 위스콘신 유방암 데이터셋 컬럼 설명
- 이 데이터는 종양의 세포 핵(nucleus) 모양을 여러 지표로 표현한 것. 
- 각 feature는 다음 10개의 기본 값에 대해 아래와 같이 저장함.
    - mean : 평균값 저장
    - error : 표준오차(측정값의 변동성) 저장
    - worst : 가장 안 좋은(최대) 값 근처의 평균 저장
...
```
- 그 외 추가코드 확인: https://github.com/ilovejwoo1004-bit/AI_advanced/blob/main/%EB%A8%B8%EC%8B%A0%EB%9F%AC%EB%8B%9D/XGBoost%20%26%20SHAP2.ipynb
