...
### [10] [중요] 모든 오디오 파일에 대해 Spectrogram 이미지 저장 (matplotlib,librosa)
...
def save_spectrogram_image(audio_path, save_path, sr=SR, duration=DURATION):
    ...
    # png로 저장
    plt.savefig(save_path, box_inches="tight", pad_inched=0)
    ...
    img = Image.resize((IMG_WIDTH, IMG_HEIGHT)) # 이 크기만큼 학습 데이터 크기 저장
    img.save(save_path)
```
