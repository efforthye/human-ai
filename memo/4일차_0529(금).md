...
```
bad_loan_status_to_grades.sort_index().to_csv('./data/3-05)bad_loan_status_교수님.csv', sep=',')
```
...
df['date'] = pd.to_datetime(df.date)
## 월 단위 매출 집계 - 데이터타임이기 때문에 M 가능 - OK
```py
df_month = df.groupby(
...
## 제품 별 매출 - 이익 상관관계 (두 변수 관계) - 산점도 그래프 뽑아보기 - OK
